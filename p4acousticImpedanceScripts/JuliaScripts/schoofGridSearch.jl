...
# Select and define variable Schoof sliding law parameter (CS or cMAX)
useCS = true;
if useCS == true
   println("useCS = true: varying the Schoof friction coefficient")
   NetCDFfile = "schoofGridSearchCS.nc";
   startExp = 6;
   endExp = 13;
   elementsPerOrder = 20;
   ...
   SLVList = schoofFrictionCoeffList;
   nSLV = length(schoofFrictionCoeffList); # SLV = sliding law variable
else
   println("useCS = false: varying the maximum value of C_max=tau_b/N which is bounded by the local maximum up-slope of the bedrock")
   NetCDFfile = "schoofGridSearchCMAX.nc";
   cMAXcoeffList = (0.01:0.01:0.7);
   SLVList = cMAXcoeffList;
   nSLV = length(cMAXcoeffList);
end
...
for iSite in 1:nSites
	...
	function brondexSlidingLaw(N, uB, tauB)
		 if useCS == true
		    return tauB - schoofFrictionCoeffList[iSLV] * uB^m / (1 + (schoofFrictionCoeffList[iSLV]/(cMAX * N))^(1/m) * uB )^m
		 else
		    return tauB - cS * uB^m / (1 + (cS/(cMAXcoeffList[iSLV] * N))^(1/m) * uB )^m
		 end
	end
...
if useCS == true
   ncwrite(collect(schoofFrictionCoeffList), NetCDFfile, "SLVList");
else
   ncwrite(collect(cMAXcoeffList), NetCDFfile, "SLVList");
end
