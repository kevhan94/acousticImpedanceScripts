...
ncFile = NetCDF.open(inputPath*"mostLikelyVals"*des*".nc");
...
for iSite in 1:nSites
	...
	SLV = mostLikelyVals[iModel, 1];
    	grainDiameter = mostLikelyVals[iModel, 2];
   	porosity = mostLikelyVals[iModel, 3];
	println(inFileOptions[iModel],' ',SLV,' ',grainDiameter,' ',porosity)
...
nccreate(NetCDFfile, "mostLikelyVals", "models", nInFileOptions, "parameters", 3);
