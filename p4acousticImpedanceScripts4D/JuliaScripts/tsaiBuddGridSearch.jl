...
# Select and define variable Tsai-Budd sliding law parameter (mu or Cb)
useCFC = false;
if useCFC == true
   println("useCFC = true: varying the Coulomb friction coefficient")
   NetCDFfile = "tsaiBuddGridSearchCFC.nc";
...
for iSite in 1:nSites
    ...
    # Loop over sliding law parameter (mu or Cb)
    for iSLV=1:nSLV

    	# Calculate effective pressure (this assumes q=1)
	if useCFC == true
	   effectivePressure = tauBedSmooth3ptTri ./ min.(coulombFrictionCoeffList[iSLV],Cb .* uBedSmooth3ptTri.^m);
	else
	   effectivePressure = tauBedSmooth3ptTri ./ min.(mu,buddFrictionParmList[iSLV] .* uBedSmooth3ptTri.^m);
	end
