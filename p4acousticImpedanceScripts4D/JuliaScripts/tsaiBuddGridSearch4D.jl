...
# Select and define variable Tsai-Budd sliding law parameters (mu and Cb)
println("varying the Coulomb friction coefficient and the Budd friction parameter")
NetCDFfile = "tsaiBuddGridSearchCFCBFP"*des*".nc";
...
nBFP = length(buddFrictionParmList);

# Define Tsai-Budd sliding law parameters
m = 1.0/3.0;
q = 1.0;

# Set up output arrays (misfit)
dof=zeros(nPorosity,nGrainDiam,nCFC,nBFP);
chiSq=zeros(nPorosity,nGrainDiam,nCFC,nBFP);
...
# Loop over all sites
icount = 0;
ncount = nPorosity * nGrainDiam * nCFC * nBFP * nSites;
...
for iSite in 1:nSites
    ...
    # Loop over sliding law parameter (mu and Cb)
    for iCFC=1:nCFC
    	for iBFP=1:nBFP

    	    # Calculate effective pressure (this assumes q=1)
	    effectivePressure = tauBedSmooth3ptTri ./ min.(coulombFrictionCoeffList[iCFC], buddFrictionParmList[iBFP] .* uBedSmooth3ptTri.^m);
	    ...
	    pressureExceeds = effectivePressure .> overburdenPressure;
            if count(pressureExceeds) > 0
               ...
               println("Site: ", iSite, ", CFC: ", coulombFrictionCoeffList[iCFC], ", BFP: ", buddFrictionParmList[iBFP])
	    ...
	    # Loop over VGS parameter space
		    ...
		    # Store misfit
		    dof[iPorosity,iGrainDiam,iCFC,iBFP]=dof[iPorosity,iGrainDiam,iCFC,iBFP]+thisDoF;
            	    chiSq[iPorosity,iGrainDiam,iCFC,iBFP]=chiSq[iPorosity,iGrainDiam,iCFC,iBFP]+thisChiSq;
...
outputData = zeros(nPorosity,nGrainDiam,nCFC,nBFP,nVars);
outputData[:,:,:,:,1] = dof;
outputData[:,:,:,:,2] = chiSq;
for iVars=1:nVars
    nccreate(NetCDFfile, outputVars[iVars], "porosity", nPorosity, "grainDiameter", nGrainDiam, "CFC", nCFC, "BFP", nBFP);
...
nccreate(NetCDFfile, "BFPList", "BFP", nBFP);
...
ncwrite(collect(buddFrictionParmList), NetCDFfile, "BFPList");
