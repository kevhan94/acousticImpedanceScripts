...
inputPath4D=""+preFix
outputPath=""+preFix
netcdfPath = inputPath4D+"mostLikelyVals4D"+des+".nc"
...
def readData(iInFile,inFileOptions):
    ...
    elif inFileOptions[iInFile] == "tsaibuddCFCBFP":
        NetCDFfile = inputPath4D+"tsaiBuddGridSearchCFCBFP.4D.nc"
    elif inFileOptions[iInFile] == "zoetCFCuTnoN":
        NetCDFfile = inputPath4D+"zoetGridSearchCFCuTnoN.4D.nc"
    elif inFileOptions[iInFile] == "schoofCSCMAX":
    ...
    dof = d.variables['dof'][:].filled(np.nan)
    chiSq = d.variables['chiSq'][:].filled(np.nan)
    if inFileOptions[iInFile] not in ["weertmanN0","weertmanNpi","tsaibuddCFCBFP","zoetCFCuTnoN","schoofCSCMAX"]:
        ...
        SLVList = d.variables['CFCList'][:]
        SLV2List = d.variables['BFPList'][:]
    elif inFileOptions[iInFile] == "zoetCFCuTnoN":
        SLVList = d.variables['CFCList'][:]
        SLV2List = d.variables['uTList'][:]
    elif inFileOptions[iInFile] == "schoofCSCMAX":
        SLVList = d.variables['CSList'][:]
        SLV2List = d.variables['cMAXList'][:]
    ...
    nanCount = np.isnan(chiSq).sum()
    if nanCount > 0:
        print("Number of NaNs (chiSq):", nanCount, f"({nanCount/chiSq.size*100:.2f}%)")
        dof[np.isnan(chiSq)] = np.nan
        print("NaN values are ignored in likelihood and therefore posterior calculation")

    # to actually get chiSq, divide by number of samples
    chiSq = chiSq / dof 

    return chiSq, porosityList, grainDiameterList, SLVList, SLV2List, nanCount
...
def defPriors(iInFile, inFileOptions, uniformPriors, porosityList, grainDiameterList, SLVList, SLV2List):
    if uniformPriors == True:
        ...
        if inFileOptions[iInFile] not in ["rob","weertman","weertmanN0","weertmanNpi","tsaibuddCFCBFP","zoetCFCuTnoN","schoofCSCMAX"]:
            SLVPrior = np.ones(len(SLVList))/np.sum(np.ones(len(SLVList)))
        elif inFileOptions[iInFile] in ["tsaibuddCFCBFP","zoetCFCuTnoN","schoofCSCMAX"]:
            SLVPrior = np.ones(len(SLVList))/np.sum(np.ones(len(SLVList)))
            SLV2Prior = np.ones(len(SLV2List))/np.sum(np.ones(len(SLV2List)))
    else:
        ...
        if inFileOptions[iInFile] in ["schoofCSCMAX"]:
            SLVList *= 1e-6 * 1e-1
        if inFileOptions[iInFile] in ["zoetuTnoN"]:
            SLVList *= 1e6 * (60*60*24*365)
        if inFileOptions[iInFile] in ["zoetCFCuTnoN"]:
            SLV2List *= 1e6 * (60*60*24*365)
        ...
        if inFileOptions[iInFile] in ["coulomb","tsaibuddCFC","zoet"]:
            ...
            pass
        elif inFileOptions[iInFile] in ["tsaibuddCFCBFP"]:
            shapeGamma = 3.2
            scaleGamma = 0.1
            SLVPrior = np.flip(priorGamma(SLVList, shapeGamma, scaleGamma))
            SLV2Prior = priorUniform(SLV2List)
        elif inFileOptions[iInFile] in ["zoetCFCuTnoN"]:
            shapeGamma = 3.2
            scaleGamma = 0.1
            SLVPrior = np.flip(priorGamma(SLVList, shapeGamma, scaleGamma))
            SLV2Prior = priorUniform(SLV2List)
        elif inFileOptions[iInFile] in ["schoofCSCMAX"]:
            shapeGamma = 3.2
            scaleGamma = 0.1
            SLVPrior = priorUniform(SLVList)
            SLV2Prior = priorGamma(SLV2List, shapeGamma, scaleGamma)
        else:
            print("You forgot to define a prior")

    # calculate n-dimensional parameter prior
    if inFileOptions[iInFile] in ["tsaibuddCFCBFP","zoetCFCuTnoN","schoofCSCMAX"]:
        parametersPrior = np.zeros((len(porosityPrior),len(grainDiameterPrior),len(SLVPrior),len(SLV2Prior)))
        for i in range(len(porosityPrior)):
            for j in range(len(grainDiameterPrior)):
                for k in range(len(SLVPrior)):
                    for l in range(len(SLV2Prior)):
                        parametersPrior[i,j,k,l] = porosityPrior[i]*grainDiameterPrior[j]*SLVPrior[k]*SLV2Prior[l]
    elif inFileOptions[iInFile] not in ["weertmanN0","weertmanNpi"]:
        parametersPrior = np.zeros((len(porosityPrior),len(grainDiameterPrior),len(SLVPrior)))
        for i in range(len(porosityPrior)):
            for j in range(len(grainDiameterPrior)):
                for k in range(len(SLVPrior)):
                    parametersPrior[i,j,k] = porosityPrior[i]*grainDiameterPrior[j]*SLVPrior[k]
    else:
...
mostLikelyVals = np.zeros((len(inFileOptions),4))*np.nan
for s in range(len(inFileOptions)):
    chiSq, porosityList, grainDiameterList, SLVList, SLV2List, nanCount = readData(s,inFileOptions)
    ...
    SLV2ListOrg = np.copy(SLV2List)
    parametersPrior = np.transpose(defPriors(s, inFileOptions, uniformPriors, porosityList, grainDiameterList, SLVList, SLV2List))
    if nanCount > 0:
        print("Set prior to 0 where misfit is NaN and rescale (should still integrate to 1)")
        parametersPrior[np.isnan(chiSq)] = 0
        print(f"sum(parametersPrior): {np.sum(parametersPrior):.2f}")
        parametersPrior /= np.sum(parametersPrior)
        print(f"sum(parametersPrior): {np.sum(parametersPrior):.2f}")
    ...
    likelihoods[s] = np.nansum(chiSq)
    posteriorDist = np.exp(-0.5*chiSq)*parametersPrior # 0.5 assumes a gaussian distribution of the errors of the data
    posteriors[s] = np.nansum(posteriorDist)

    print("posteriorDist",np.shape(posteriorDist))
    # Get most likely values
    if inFileOptions[s] in ["weertmanN0","weertmanNpi"]:
        mostLikelyInds = np.unravel_index(np.nanargmax(posteriorDist),posteriorDist.shape)
        mostLikelyVals[s,2:] = [grainDiameterListOrg[mostLikelyInds[0]],porosityListOrg[mostLikelyInds[1]]]
    elif inFileOptions[s] in ["tsaibuddCFCBFP","zoetCFCuTnoN","schoofCSCMAX"]:
        mostLikelyInds = np.unravel_index(np.nanargmax(posteriorDist),posteriorDist.shape)
        mostLikelyVals[s,:] = [SLV2ListOrg[mostLikelyInds[0]],SLVListOrg[mostLikelyInds[1]],grainDiameterListOrg[mostLikelyInds[2]],porosityListOrg[mostLikelyInds[3]]]
    else:
        mostLikelyInds = np.unravel_index(np.nanargmax(posteriorDist),posteriorDist.shape)
        mostLikelyVals[s,1:] = [SLVListOrg[mostLikelyInds[0]],grainDiameterListOrg[mostLikelyInds[1]],porosityListOrg[mostLikelyInds[2]]]
    print("#--------------------------------------------------------------------------------------#")

print("Most likely values: SLV2,SLV,grainDiameter,porosity")
print(mostLikelyVals)

if uniformPriors == True:
    netcdfPath = netcdfPath[:-3]+'UniformPriors.nc'
print("Save most likely SlV2, SLV, grain diameter, and porosity values in",netcdfPath)
...
inFileOptions =np.where(inFileOptions=="weertmanN0",r"$N = 0~\mathrm{Pa}$",inFileOptions)
inFileOptions =np.where(inFileOptions=="weertmanNpi",r"$N = P_\mathrm{i}$",inFileOptions)
...
inFileOptions =np.where(inFileOptions=="tsaibuddCFCBFP",r"Tsai-Budd ($C_{\mathrm{B}}$, $\mu$)",inFileOptions)
inFileOptions =np.where(inFileOptions=="schoofCS",r"Schoof ($C_{\mathrm{S}}$)",inFileOptions)
inFileOptions =np.where(inFileOptions=="schoofCMAX",r"Schoof ($C_{\mathrm{max}}$)",inFileOptions)
inFileOptions =np.where(inFileOptions=="schoofCSCMAX",r"Schoof ($C_{\mathrm{S}}$, $C_{\mathrm{max}}$)",inFileOptions)
inFileOptions =np.where(inFileOptions=="zoet",r"Zoet-Iverson ($\mu$)",inFileOptions)
inFileOptions =np.where(inFileOptions=="zoetuTnoN",r"Zoet-Iverson ($C_{\mathrm{ZI}}$)",inFileOptions)
inFileOptions =np.where(inFileOptions=="zoetCFCuTnoN",r"Zoet-Iverson ($C_{\mathrm{ZI}}$, $\mu$)",inFileOptions)
...
figName = "bayesTheoremPostComp4D"
figNameMin= "bayesTheoremPostCompMin4D"
addOn = 'NobCS'
if uniformPriors == True:
